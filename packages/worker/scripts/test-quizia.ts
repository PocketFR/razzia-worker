...
const cas: [string, () => void | Promise<void>][] = []
...
t("identifiant : 21 caractères de l'alphabet razzia", () => {
  ...
  assert.strictEqual(new Set([...Array(500)].map(() => identifiant())).size, 500)
})

// ── slug ───────────────────────────────────────────────────────────────────
t("slug : accents, ponctuation, troncature, repli", () => {
  assert.strictEqual(slug("Blind Test Rock français 80s"), "blind-test-rock-francais-80s")
...
t("validerQuestion : cinq réponses écartées (schéma razzia)", () => {
  const [q, motif] = validerQuestion({ q: "Qui ?", a: ["a", "b", "c", "d", "e"], s: 0 }, 7)
  assert.strictEqual(q, null)
  assert.match(String(motif), /5 réponses, maximum 4/)
})

t("validerQuestion : rejets divers", () => {
  assert.strictEqual(validerQuestion({ q: "", a: ["a", "b"], s: 0 }, 1)[0], null)
  assert.strictEqual(validerQuestion({ q: "x", a: ["a"], s: 0 }, 1)[0], null)
  assert.strictEqual(validerQuestion({ q: "x", a: ["a", "b"], s: 5 }, 1)[0], null)
...
t("validerQuestion : n et start", () => {
  const [q] = validerQuestion({ q: "x", a: ["a", "b"], s: 0, n: 3, start: 60 }, 1)
...
// ── ecrireQuiz ─────────────────────────────────────────────────────────────
const pistes = [
  { id: "aaaaaaaaaaaaaaaaaaaaaa", artiste: "Téléphone", titre: "Cendrillon", annee: 1982 },
  { id: "bbbbbbbbbbbbbbbbbbbbbb", artiste: "Indochine", titre: "L'Aventurier", annee: 1982 },
  { id: "cccccccccccccccccccccc", artiste: "Images", titre: "Les Démons de minuit", annee: 1986 },
]

const quizEcrit = (lignes: any[]) => JSON.parse(lignes[0][2])

t("ecrireQuiz : liaison par numéro, culture sans média, document valide", async () => {
  const { db, lignes } = faireBase()
  const r = await ecrireQuiz(db, cles, "Blind Test années 80", [
    { q: "Quel titre ?", a: ["Cendrillon", "Un autre monde", "Ça"], s: 0, n: 1, start: 30 },
    { q: "Quel groupe ?", a: ["Indochine", "Taxi Girl", "Kas Product"], s: 0, n: 2 },
    { q: "Capitale du Pérou ?", a: ["Lima", "Quito", "La Paz", "Bogota"], s: 0 },
  ], pistes)
  ...
  assert.strictEqual(doc.questions[0].media.url, "spotify:aaaaaaaaaaaaaaaaaaaaaa:30")
  assert.strictEqual(doc.questions[1].media.url, "spotify:bbbbbbbbbbbbbbbbbbbbbb")
  ...
  assert.strictEqual(doc.questions[0].answers[doc.questions[0].solutions[0]], "Cendrillon")
  assert.strictEqual(doc.questions[2].answers[doc.questions[2].solutions[0]], "Lima")
})

t("ecrireQuiz : repli sur artiste et titre quand « n » manque", async () => {
  const { db, lignes } = faireBase()
  await ecrireQuiz(db, cles, "Repli", [
    {
      q: "Quel titre ?",
  ...
  assert.strictEqual(quizEcrit(lignes).questions[0].media.url, "spotify:cccccccccccccccccccccc")
})

t("ecrireQuiz : question musicale non résolue écartée", async () => {
  const { db } = faireBase()
  const r = await ecrireQuiz(db, cles, "Introuvable", [
...
t("ecrireQuiz : même morceau deux fois, seconde écartée", async () => {
  const { db } = faireBase()
  const r = await ecrireQuiz(db, cles, "Doublon", [
    { q: "A ?", a: ["x", "y"], s: 0, n: 1 },
    { q: "B ?", a: ["x", "y"], s: 0, n: 1 },
  ], pistes)
...
t("ecrireQuiz : plafond de 40 questions", async () => {
  const { db } = faireBase()
  const brutes = [...Array(50)].map((_, i) => ({ q: `Q${i}`, a: ["a", "b"], s: 0 }))
